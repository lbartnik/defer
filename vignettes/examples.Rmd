...
Now we can call `deferred_fun()` completely ommitting arguments.
See `?defer::augment` for more details.
...
two other packages: [base64enc](https://cran.r-project.org/package=base64enc)
to serialize the wrapper and [httr](https://cran.r-project.org/package=httr)
to talk to OpenCPU's HTTP-based API.
...
script, `local_script_path`, as its argument. We will be calling a public
...
Here is what a `httr` call to `opencpu` looks like:
...
The script contains `deferred_fun` serilized into a base64-encoded string.
It instructs the remote R session to load the `base64enc` package, deserialize
our wrapper, and then run it in that remote R session.

```{r to_base64, echo=FALSE}
...
Here is the `to_base64()` function which uses `base64enc`'s API to
serialize its argument.
